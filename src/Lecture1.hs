...
minmax x y z = maximum list - minimum list
  where
      list = [x, y, z]
...
foo :: Integer -> [Integer] -> (Integer -> Integer -> Bool) -> Integer
foo _      []     _ = 0
foo number (x:xs) f = foo number xs f + bar
  where
      bar = if f number x then 1 else 0
